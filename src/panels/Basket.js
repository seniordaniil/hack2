...
const Basket = ({
  ...
  const [faster, setFaster] = useState(
    orderData[itemId] ? orderData[itemId].faster : true
  );
  const [time, setTime] = useState(
    orderData[itemId] ? orderData[itemId].time : ''
  );
  const [selfService, setSelfService] = useState(
    orderData[itemId] ? orderData[itemId].selfService : false
  );
  const area = foodAreas.filter((area) => area.id === areaId)[0];
  const item = area.items.filter((item) => item.id === itemId)[0];

  useEffect(() => {
    const odata = getOrderData();
    odata[itemId] = { time, faster, selfService };
    localStorage.setItem('orderData', JSON.stringify(odata));
  }, [faster, time, selfService, itemId]);
